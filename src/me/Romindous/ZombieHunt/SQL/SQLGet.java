...
public class SQLGet {
	...
	public String getString(final String name, final String cat, final String tbl) {
		try {
			...
			return rs.next() ? rs.getString(cat.toUpperCase()) : "";
		} catch (SQLException e) {
			e.printStackTrace();
			return "";
		}
	...
	public void chckIfExsts(final String name, final String tbl) {
		try {
			final ResultSet rs = exctStrStmt("SELECT * FROM " + tbl + " WHERE NAME=?", name).executeQuery();
			if (rs.next() || name.startsWith("guest")) return;
			...
			ps.setString(1, name);
			ps.setString(2, (String) kits.getConfigurationSection("kits.zombie").getKeys(false).iterator().next());
			ps.setString(3, (String) kits.getConfigurationSection("kits.player").getKeys(false).iterator().next());
			ps.setInt(4, 0);
			ps.setInt(5, 0);
			ps.setInt(6, 0);
			ps.setInt(7, 0);
